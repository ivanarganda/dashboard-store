...
export default function ShoppingCart(props) {
  ...
  const [totalPrice, setTotalPrice] = useState(cart.totalPriceCart);
  ...
  useEffect(() => {
    ...
    const subtotal = itemsShopping.reduce((acc, item) => {
      return acc + item.quantity * item.specifications.price;
    }, 0);
  ...
  return (
    <>
      <div className='mt-20 sm:mt-12 md:mt-24 lg:mt-13'></div>
      {itemsShopping.length !== 0 && (
        <article className='w-1/4 flex flex-row justify-center items-center rounded-xl shadow-xl flex bg-[#1F1D2B] min-w-[350px] m-auto'>
          <form className='flex w-full flex-col p-6 min-w-fit' onSubmit={purchageForm}>
            ...
            <div className='flex flex-col relative justify-center w-full min-w-fit mt-2'>
              <span className='flex flex-row justify-start items-start w-full m-auto text-gray-300 text-md mb-1'>Subtotal price: {subtotalPrice} €</span>
            </div>
            <div className='flex flex-col relative justify-center w-full min-w-fit mt-2'>
            ...
            return (
              <>
                <article className='h-full md:-ml-0 xl:-ml-40 flex flex-col justify-center items-center pb-3' key={item.id}>
                  <div className='bg-gray-200 pb-2 bg-opacity-10 w-2/3 min-w-[350px] shadow-2xl mt-20 h-full relative flex flex-col justify-center items-center text-gray-400 rounded-xl'>
                        ...
                        <span>
                          <ul>
                            {especifications[item.category].map((l, idx) => {
                              ...
                              return (
                                <li key={idx} className='flex flex-row space-x-2 text-gray-200'>
                                  <CheckIcon sx={{ fontSize: '30px' }} color='success' />
                                  {l}: {item.specifications[l]}
                                </li>
                              );
                            })}
                            <li className='flex flex-row space-x-2 text-gray-200'>
                              <CheckIcon sx={{ fontSize: '30px' }} color='success' />
